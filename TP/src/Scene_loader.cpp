...
namespace OM3D {
    ...
    static bool
    decode_attrib_buffer(const tinygltf::Model &gltf, const std::string &name, const tinygltf::Accessor &accessor, Span<Vertex> vertices) {
    ...
    static void parse_node_transforms(int node_index, const tinygltf::Model &gltf, std::unordered_map<int, glm::mat4> &node_transforms,
    ...
    Result<std::unique_ptr<Scene>> Scene::from_gltf(const std::string &file_name) {
        ...
        for (auto [node_index, light_index]: light_nodes) {
            ...
            const glm::vec3 color =
                    glm::vec3(float(gltf_light.color[0]), float(gltf_light.color[1]), float(gltf_light.color[2])) * float(gltf_light.intensity);;
